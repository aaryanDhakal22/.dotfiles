...
@api_view(["GET", "POST"])
def notification_detail(request, studentunid):
    try:
        notification = StudentModel.objects.get(pk=notificationId)
    except StudentModel.DoesNotExist:
        return JsonResponse(
    ...
    if request.method == "GET":
        notification_serializer = StudentSerializers(notification)
        return JsonResponse(notification_serializer.data)
    elif request.method == "PUT":
        notification_data = JSONParser().parse(request)
        notification_serializer = StudentSerializers(notification, data=notification_data)
