...
def notification_list(request, studentId):
    if request.method == "GET":
        ...
        if studentId is not None:
            notifications = notifications.filter(studentunid__icontains=studentId)
...
@api_view(["GET", "PUT", "DELETE"])
def notification_detail(request,studentId,notificationId):
    try:
        notification = NotificationModel.objects.get(pk=notificationId)
    except NotificationModel.DoesNotExist:
        return JsonResponse(
            {"message": "The notification does not exist"}, status=status.HTTP_404_NOT_FOUND
        )
    ...
    if request.method == "GET":
        ...
        notification_data = JSONParser().parse(request)
        notification_serializer = NotificationSerializers(notification, data=notification_data)
        if notification_serializer.is_valid():
            notification_serializer.save()
            return JsonResponse(notification_serializer.data)
        return JsonResponse(
