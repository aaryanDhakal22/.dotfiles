...
@api_view(["GET", "POST"])
def transaction_list(request, studentId):
    if request.method == "GET":
        transactions = NotificationModel.objects.all()
        if studentId is not None:
            transactions = transactions.filter(transactionId__icontains=studentId)
        transaction_serializer = NotificationSerializers(transactions, many=True)
        return JsonResponse(transaction_serializer.data, safe=False)

    elif request.method == "POST":
        transaction_data = JSONParser().parse(request)
        transaction_serializer = NotificationSerializers(data=transaction_data)
...
@api_view(["GET", "POST"])
def transaction_list(request, transactionId):
    if request.method == "GET":
        transactions = TransactionModel.objects.all()
        if transactionId is not None:
            transactions = transactions.filter(transactionId__icontains=transactionId)
