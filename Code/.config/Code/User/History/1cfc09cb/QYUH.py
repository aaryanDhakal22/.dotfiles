...
@api_view(["GET", "POST"])
def notification_list(request, studentId):
    if request.method == "GET":
        notifications = NotificationModel.objects.all()
        if studentId is not None:
            notifications = notifications.filter(transactionId__icontains=studentId)
...
def transaction_detail(request, transactionId):
    try:
        transaction = NotificationModel.objects.get(pk=transactionId)
    except NotificationModel.DoesNotExist:
        return JsonResponse(
    ...
    if request.method == "GET":
        transaction_serializer = NotificationSerializers(transaction)
        return JsonResponse(transaction_serializer.data)

    elif request.method == "PUT":
        transaction_data = JSONParser().parse(request)
        transaction_serializer = NotificationSerializers(
