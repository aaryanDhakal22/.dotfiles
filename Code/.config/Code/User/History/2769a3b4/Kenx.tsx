    ...
    return (
        <>
            {notsOfStudent.data.map((notification)=>{
                return <LinedTile onClick={()=>navigate(`/notification/${notification.studentId}/details/${notification.notificationId}`)}  left={notification.date} center={notification.month} right={notification.amount} />
            })}
        </>
    )
