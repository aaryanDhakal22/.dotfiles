...
const AccountForm = ({student,toAdd}:{student:Student,toAdd:boolean})=>{
    ...
    const formik= useFormik<Student>({
        ...
        onSubmit:(values)=>{
            // const new_values = {...values,"isAdmission":values.isAdmission=="admission"?true:false}
            console.log(values)
            // if(toAdd){
            //     addStudent(values)
            // }else{
    ...
    return (
        ...
        <form onSubmit={formik.handleSubmit}>
            ...
            <input type="checkbox" onChange={formik.handleChange} name="isAdmission" id="isAdmission" value={formik.values.isAdmission?"admission":undefined} checked={formik.values.isAdmission?true:false} />
