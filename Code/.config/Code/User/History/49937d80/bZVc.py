...
def transaction_list(request):
    if request.method == 'GET':
        transactions = StudentModel.objects.all()

        name = request.GET.get('name',None)
        if name is not None:
            transactions = transactions.filter(name__icontains=name)
        student_serializer = transactionserializers(transactions,many=True)
        return JsonResponse(student_serializer.data,safe=False)


    elif request.method == "POST":
        student_data = JSONParser().parse(request)
        student_serializer = transactionserializers(data=student_data)
        if student_serializer.is_valid():
            student_serializer.save()
            return JsonResponse(student_serializer.data,status=status.HTTP_201_CREATED)
        return JsonResponse(student_serializer.errors,status=status.HTTP_400_BAD_REQUEST)